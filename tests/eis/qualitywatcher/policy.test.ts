...
const test = base.extend<MyFixtures>({
  ...
  customer: [async ({ ratingPage, customerPage }, use) => {
...
test("[S11C1976] Verify that the new TRN field on DRIVER tab can't be left blank when SAVE & EXIT button is selected",
  async ({ policyPage, customer, page }) => {
    await policyPage.startNewQuote();
    await policyPage.selectOptionForField(policyPage.branchField, 'Head Office - Kingston');
    await policyPage.selectOptionForField(policyPage.currencyField, 'JMD');
    await policyPage.clickDriverTab();
...
test("[S11C1999] Verify Reason for Override field is required and dropdown values are correct",
  async ({ policyPage, customer }) => {
    ...
    await policyPage.click(policyPage.calculatePremiumButton);
    await policyPage.waitForLoadingSpinner();
...
test("[S11C2000] Verify error message when Reason for Override is left blank and Save button is selected",
  async ({ policyPage, customer }) => {
    ...
    await policyPage.fillRequiredVehicleInformation(
      vehicleRequiredInformation
    );

    await policyPage.clickPremiumsAndCoveragesTab();

    await policyPage.selectOptionForCoverageTypeField(
      selectedCoverageType
    );

    await policyPage.selectOptionForPlanSelection(
    ...
    // Select vehicle row from Premium Summary
    ...
    await policyPage.click(
      policyPage.overrideCoveragePremiumButton
    );

    await policyPage.click(
      policyPage.page
        .locator("a:right-of(:text('Third Party Bodily Injury'))")
        .getByText('Select')
        .first()
    );
    ...
    await policyPage.verifyErrorMessageIsVisible(
      policyPage.reasonForOverrideErrorMessage
    );
  })
...
test("[S11C2031] Failed: Verify that \"Endorsement Reason\" field is displayed with None/Blank and it is a mandatory field when you start an endorsement",
  async ({ policyPage, page, ratingPage, customer }) => {
    ...
    if (!createdPolicyNumber) throw new Error('Policy number not found');
...
test("[S11C2037] Verify that the \"Smallz\" plan is included in the Plan Selection drop-down when Private Car Third Party coverage type is selected for an endorsement quote",
  async ({ policyPage, page, customer, ratingPage }) => {
    ...
    const policyNumberText = await policyPage.policyNumberText.textContent() || '';
    const createdPolicyNumber = policyNumberText.replace('#', '').trim();
    if (!createdPolicyNumber) throw new Error('Policy number not found');

    const notCurrentPolicyVersionMessage = page.getByText(
      'Policy version you are working with is marked as NOT current',
      { exact: false }
    );

    const startEndorsement = async () => {
      await policyPage.takeActionDropdown.selectOption('endorseWithWorkspace');
      await policyPage.waitForLoadingSpinner();
    };

    await startEndorsement();

    if (await notCurrentPolicyVersionMessage.isVisible({ timeout: 5_000 }).catch(() => false)) {
      await page.locator('#validateActionPopup_header_controls span.icon-close[title="close"]').click();
      await policyPage.waitForLoadingSpinner();
      await policyPage.logout();
      await ratingPage.login(process.env.EIS_USERNAME!, process.env.EIS_PASSWORD!);
      await ratingPage.waitForLoadingSpinner();
      await ratingPage.searchCustomer(customer.customerId);
      await ratingPage.clickPolicyNumberLink(createdPolicyNumber);
      await startEndorsement();
    }

    await policyPage.endorsementDateField.fill(new Date().toLocaleDateString('en-GB'));
    await page.keyboard.press('Enter');
    await policyPage.waitForLoadingSpinner();
    await policyPage.endorsementReasonField.selectOption({ label: 'Changing coverage type/plan selection' });
    await policyPage.waitForLoadingSpinner();
...
test("[S11C2038] Verify that the \"Smallz\" plan is included in the Plan Selection drop-down when Private Car Third Party coverage type is selected for manual renewal quote",
  async ({ policyPage, customer, ratingPage, page }) => {
    await policyPage.startNewQuote();
    await policyPage.selectPolicyCounty('Jamaica');
    await policyPage.selectBranch('Head Office - Kingston');
    await policyPage.checkPremiumFincancing('No');
    ...
    await page.locator('#policyDataGatherForm\\:yesBtn_PolicyLapseRenewAction_footer').click();
    await policyPage.waitForLoadingSpinner();
...
test("[S11C2049] Verify that selecting 'Standard w Rental Benefits' displays the correct vehicle coverages and limits.",
  async ({ policyPage, customer, ratingPage }) => {
    ...
    await policyPage.clickCalculatePremiumButton();
    ...
    await expect(policyPage.page.getByRole('combobox', { name: 'Excess Limit' })).toHaveValue('FIVEMIN15000MAX350KJMD');
...
test("[S11C2058] Verify that the Jamaica technical agency (Tech-Agency-JM) is assigned to a customer with a Jamaica address.",
  async ({ policyPage, ratingPage, customerPage }) => {
    await ratingPage.login(process.env.EIS_USERNAME!, process.env.EIS_PASSWORD!);
    await customerPage.createNewCustomer(40, 'Jamaica');

    await policyPage.startNewQuote();
    const agencyProducerRow = policyPage.page.locator('#policyDataGatherForm\\:formGrid_ProducerInfo-0');
    await expect(agencyProducerRow.locator('label')).toHaveText('Agency/Producer');
    await expect(agencyProducerRow.locator('label')).toBeVisible();
  })
...
test.describe.serial("Duplicate VIN Rule", async () => {
  ...
  test.beforeEach(async ({ ratingPage, customerPage }) => {
    ...
    if (!vinAlreadyExists && !policyCreated) {
      // VIN doesn't exist - create full policy
      ...
      // VIN already exists - no need to create policy
    } else {
      // Policy already created in previous test
    }
  })

  test.skip("[S11C2064] Rule is triggered when clicking 'Next'",
    async ({ policyPage, customerPage, ratingPage }) => {
      await ratingPage.login(process.env.EIS_USERNAME!, process.env.EIS_PASSWORD!);

      const duplicateVehicleRequiredInformation = {
        ...vehicleRequiredInformation,
        generalInformation: {
          ...vehicleRequiredInformation.generalInformation,
          chassisVIN: faker.vehicle.vin()
        }
      };

      // Create an issued policy first so the VIN exists in the system.
      await customerPage.clickCustomerTabButton();
      const existingPolicyCustomer = await customerPage.createNewCustomer(40, 'Jamaica');
      ...
      await policyPage.headerNextButton.click();
      await policyPage.waitForLoadingSpinner();
      await policyPage.selectInsuredParty(existingPolicyCustomer.customerName, 'Advantage General Insurance Company');
      await policyPage.goToNextTab('Driver');
      await policyPage.selectExistingDriver(existingPolicyCustomer.customerName, 'Permanent', 'Valid');
      await policyPage.goToNextTab('Vehicle');
      await policyPage.clickAddNewVehicleButton();
      await policyPage.waitForLoadingSpinner();
      await policyPage.fillRequiredVehicleInformation(duplicateVehicleRequiredInformation);
      await policyPage.clickPremiumsAndCoveragesTab();
      await ratingPage.setCoverageAndPlan('Comprehensive');
      await ratingPage.calculatePremium();
      await ratingPage.clickFundingSummaryTab();
      ...
      await policyPage.handlePurchasePolicyConfirmation(true);
      await policyPage.finishPayment({
        billingAccountName: existingPolicyCustomer.customerName,
        city: 'Test City'
      });
      await policyPage.waitForLoadingSpinner();
  ...
  test.skip("[S11C2065] Verify error page opens when clicking 'Override Rules'",
    async ({ policyPage, customerPage, ratingPage }) => {
      await ratingPage.login(process.env.EIS_USERNAME!, process.env.EIS_PASSWORD!);
      ...
      await policyPage.overrideRulesButton.click();
      await policyPage.waitForLoadingSpinner();
      await expect(policyPage.page.locator(':text-matches("Error")').first()).toBeVisible();
      await expect(policyPage.page.locator(':text-matches("This Chassis/VIN is already in use and cannot be added.", "i")').first()).toBeVisible();
    })

  test("[S11C2066] Failed: Rule is triggered at the 'Purchase' step",
    async ({ policyPage, customerPage }) => {
...
test("[S11C2070] Verify the 'Reason' field default display is Blank/none and contains specified dropdown values when Manual Renew is selected.",
  async ({ policyPage, page, customer, ratingPage }) => {
    ...
    await policyPage.verifyFieldIsPresentEditableAndDefaultIsBlank(policyPage.manualReviewReasonField);
...
test("[S11C2074] Verify the system displays an error message when the 'Do Not Renew Status' field is left blank during Do Not Renew.",
  async ({ policyPage, page, customer, ratingPage }) => {
    ...
    const startDoNotRenew = async () => {
      await policyPage.takeActionDropdown.selectOption('doNotRenew');
      await policyPage.waitForLoadingSpinner();
    };

    await startDoNotRenew();
