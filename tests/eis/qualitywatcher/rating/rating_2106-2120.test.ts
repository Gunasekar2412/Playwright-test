...
test('[S11C2106] Verify the Rater updates the premium step-by-step as the NFD discount is applied', async () => {
    ...
    await expect(insurancePremium).toBeGreaterThan(premium1Year.premium);
    ...
    await ratingPage.selectInsuredParty(customerName, data.insuredParty.priorCarrier, true, '3YEARS');
    await ratingPage.clickPremiumsAndCoveragesTab();
    await ratingPage.calculatePremium();
    ...
    await expect(insurancePremium).toBeGreaterThan(premium3Year.premium);});
...
test('[S11C2107] Verify the Rater calculates NFD using the insured with the highest "Number of years driving claim free?" value', async () => {
    ...
    // Set insured party details
    await ratingPage.selectInsuredParty(customerName, data.insuredParty.priorCarrier, true, 'MORE5');
    ...
    // Verify the premium
    let insurancePremium = await ratingPage.getPremiumValue();    expect(insurancePremium).toBeGreaterThan(result.premium);
});
...
test('[S11C2111] Verify the Rater calculates GPS Discount when Tracking System is selected', async () => {
    ...
    await ratingPage.goToNextTab('Driver');
    await ratingPage.selectExistingDriver(
        customerName, driver.licenseType, driver.licenseStatus
    );
...
test('[S11C2112] Verify the Rater calculates the applicable discount combinations', async () => {
    ...
    await ratingPage.goToNextTab('Driver');
    await ratingPage.selectExistingDriver(
        customerName, data.driver.licenseType, data.driver.licenseStatus
    );
    await ratingPage.goToNextTab('Vehicle');
    await ratingPage.addNewVehicle(vehicle);
    await ratingPage.clickPremiumsAndCoveragesTab();
...
test('[S11C2113] Verify the Rater calculates Driver Age discount based on the driver\'s age. Driver = over 60 years old', async () => {
    ...
    expect(premiumOver60).toBeLessThan(premiumAge40 * 0.8);});
...
test.skip('[S11C2117] Bundle Discount is applied to the second and subsequent vehicle (Barbados, no active policy)', async () => {
    test.setTimeout(520_000);
    ...
    await ratingPage.selectVehicle('Honda, City'); // select second vehicle (index 1)
    await ratingPage.setCoverageAndPlan("Comprehensive");
...
test('[S11C2118] Bundle Discount is applied when customer has at least one active policy (Jamaica)', async () => {
    ...
    expect(Math.abs(actualPremium - expected.premium)).toBeGreaterThan(tolerance);
});
...
test('[S11C2119] Bundle Discount is NOT applied when customer has no active policy (Jamaica)', async () => {
    ...
    // Create a new customer
    const { customerName, customerDetails } = await customerPage.createNewCustomer(40, 'Jamaica');
    await ratingPage.startNewQuote();
    ...
    const expected = calculateExpectedPremium({
        ...
        options: {}
    });    expect(actualPremium).toBeGreaterThan(expected.premium);
});

test.skip('[S11C2120] Bundle Discount is applied to the second and subsequent vehicle (Jamaica, no active policy)', async () => {
