...
import { issueJamaicaPrivateMotorPolicyViaDxp } from '../../../../lib/dxp/issueJamaicaPrivateMotorPolicy';
...
test.describe('Jamaica Certificate Code Scenarios', () => {
  ...
  test('S11C2137 - Verify that Certificate is correct for a single Insured Private Car Comprehensive coverage type and Standard plan', async () => {
    test.skip(
      ...
      'DXP API setup: set DXP_API_BASE_URL, DXP_GUEST_BASIC_*, and DXP_AGENT_BASIC_* (or EIS_USERNAME/PASSWORD). Tune DXP_JM_* lookup codes for your environment.'
    );
    const { customerId, policyNumber } = await issueJamaicaPrivateMotorPolicyViaDxp({
      customerAge: 40,
      packageCd: 'StandardWORentalBenefits',
    });
  ...
  test('S11C2150 - Verify that Certificate is correct for a single Insured Private Car Comprehensive coverage type and Diamond Max plan', async () => {
    ...
    const { customerId, policyNumber } = await issueJamaicaPrivateMotorPolicyViaDxp({
      customerAge: 65,
      packageCd: 'DiamondMaxWORentalBenefits',
    });
