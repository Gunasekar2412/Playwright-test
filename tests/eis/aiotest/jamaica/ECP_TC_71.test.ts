...
test(
    'Verify claim core functionalities for a Private Motor Policy - Jamaica',
    { tag: '@ECP-TC-71' },
    async ({ page }) => {
        const lossDescription =
            `Jamaica private motor claim loss ${Date.now()}`;
        ...
        const createdPolicy: BillingCapabilityPolicy = await test.step(
            'Test setup - Create an active Jamaica Private Motor policy',
        ...
        await test.step('Step 5 - Fill Reporting Party address', async () => {
            await claimPage.fillPrivateMotorReportingPartyAddress('Jamaica');
        });
        ...
        await test.step('Step 7 - Fill Vehicle details', async () => {
            await claimPage.fillPrivateMotorVehicleDamageDetails('Jamaica');
        });
        await page.locator(
        ...
        await test.step(
            ...
            async () => {
                ...
                await claimPage.fillClaimPaymentDetails({
                    ...
                    paymentMemo:
                        `Jamaica private motor payment ${Date.now()}`,
        ...
        await test.step(
            'Step 15 - Login with QA credentials and verify Claim details',
            async () => {
