...
test('Validate commission for renewal of a Private Motor Broker policy - Jamaica', { tag: '@ECP-TC-63' }, async ({ page }) => {
    ...
    const policy = await test.step('Create backdated Jamaica Broker policy', () =>
        createJamaicaPrivateMotorPolicy(
    ...
    executionContext.customerDetails = `Broker: ${producer.name}; Renewal Expiry Date: ${expiryDate}`;
    ...
    await test.step('Verify renewal Broker commission and purchase', async () => {
