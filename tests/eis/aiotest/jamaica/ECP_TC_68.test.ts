...
import { createCommercialLiabilityPolicyForBilling } from '../../../../sites/eis/aiotest/helpers/billingCapabilityPolicyFactories';
import { recordBillingPolicy, verifyJamaicaBillingCapabilities } from './billingCapabilityTestUtils';

test.setTimeout(720_000);

test('Verify billing capabilities for a Commercial Liability Policy - Jamaica', { tag: '@ECP-TC-68' }, async ({ page }) => {
    ...
    const policy = await test.step('Purchase a Jamaica Commercial Liability policy', () =>
        createCommercialLiabilityPolicyForBilling(page, ratingPage, customerPage, policyPage, commercialPolicyPage, { region: 'Jamaica' }));
