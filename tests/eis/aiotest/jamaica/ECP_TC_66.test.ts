...
test('Verify billing capabilities for a Commercial Auto Policy - Jamaica', { tag: '@ECP-TC-66' }, async ({ page }) => {
    ...
    const policy = await test.step('Purchase a Jamaica Commercial Auto policy', () =>
        createCommercialAutoPolicyForBilling(page, ratingPage, customerPage, policyPage, commercialPolicyPage, { region: 'Jamaica' }));
