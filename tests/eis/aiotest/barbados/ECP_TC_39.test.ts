...
test(
    'Verify billing capabilities for a Commercial Liability Policy - Barbados',
    { tag: '@ECP-TC-39' },
    async ({ page }) => {
        let createdPolicy!: BillingCapabilityPolicy;

        await test.step('Step 1 - Purchase a Commercial Liability policy', async () => {
            createdPolicy = await createCommercialLiabilityPolicyForBilling(
                page,
                ratingPage,
                customerPage,
                policyPage,
