...
test(
    'Verify claim core functionalities for a Commercial Auto Policy - Barbados',
    { tag: '@ECP-TC-43' },
    async ({ page }) => {
        const createdPolicy: CommercialAutoBillingPolicy = await test.step(
            'Test setup - Create an active Commercial Auto policy',
        ...
        // Claim steps will be added from Open Claim based on the test procedure.
        void claimPage;
        const lossDescription = `Commercial auto claim loss ${Date.now()}`;
        ...
        await test.step(
            'Step 3 - Reporting Party Details Information',
            async () => {
        ...
        await test.step('Step 4 - Loss Event', async () => {
        ...
        await test.step('Step 7 - Complete Notification', async () => {
        ...
        await test.step('Step 13 - Complete Payment details', async () => {
            ...
            await claimPage.fillClaimPaymentDetails({
                ...
                customerName: createdPolicy.customerName,
                paymentMemo:
                    `Commercial auto claim payment ${Date.now()}`,
