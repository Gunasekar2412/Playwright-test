...
test(
    'Verify claim core functionalities for a Home Policy - Barbados',
    { tag: '@ECP-TC-44' },
    async ({ page }) => {
        ...
        const lossDescription = `Home claim loss ${Date.now()}`;
        const damageDescription = `Home and contents damage ${Date.now()}`;
        ...
        await test.step(
            'Step 1 - Create and purchase an active Home policy',
            async () => {
        ...
        await test.step('Step 13 - Complete Payment details', async () => {
            ...
            await claimPage.fillClaimPaymentDetails({
                ...
                grossAmount,
                customerName: createdPolicy.customerName,
                paymentMemo: `Home claim payment ${Date.now()}`,
